...
def single_fetch_content(github_user: str) -> dict:
    ...
    gh_profile_url = f"https://github.com/{github_user}"
    git_response = requests.get(gh_profile_url)
    user_dict = {
        "github_username": github_user,
        "github": Selector(text=git_response.text)
        if git_response.status_code == 200
        else None,
    }

    readme_response = requests.get(f"{gh_profile_url}/{github_user}")
    ...
    photo_url = (
        ...
        if user_dict["github"]
        else "https://i.imgur.com/PRiA9r9.png"
    )

    photo_response = requests.get(photo_url)

    gh_image_path = f"media/{github_user}_image.jpg"
